...
function constraint_dnep_power_balance(pm::AbstractLPACCModel, n::Int, i::Int, bus_arcs, bus_arcs_sw, bus_arcs_ne, bus_gens, bus_ne_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs, bus_ne_storage)
    phi  = var(pm, n, :phi, i)
    p    = get(var(pm, n),    :p, Dict()); _check_var_keys(p, bus_arcs, "active power", "branch")
    q    = get(var(pm, n),    :q, Dict()); _check_var_keys(q, bus_arcs, "reactive power", "branch")
    pg   = get(var(pm, n),   :pg, Dict()); _check_var_keys(pg, bus_gens, "active power", "generator")
...
```
"""
function constraint_ne_gen(pm::AbstractLPACCModel, n::Int, idx, pmin, pmax, qmin, qmax)
...
function constraint_gen(pm::AbstractLPACCModel, n::Int, idx, pmax)
    pg  = var(pm, n, :pg, idx)
...
function constraint_gen(pm::AbstractLPACCModel, n::Int, idx, pmin, pmax, qmin, qmax)
    pg  = var(pm, n, :pg, idx)
    pg_loss = var(pm, n, :pg_loss, idx)
    qg  = var(pm, n, :qg, idx)

    JuMP.@constraint(pm.model, pg + pg_loss == pmax)
    JuMP.@constraint(pm.model, pg >= pmin)
    JuMP.@constraint(pm.model, pg_loss >= 0)
    JuMP.@constraint(pm.model, qg <= qmax)
    JuMP.@constraint(pm.model, qg >= qmin)
end
